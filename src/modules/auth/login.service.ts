import { LoginInput, OAuthLoginInput, TokenResponse } from './requests';
...
export class LoginService {
	...
	async admin(payload: LoginInput): Promise<TokenResponse> {
		try {
	...
	async individual(payload: LoginInput): Promise<TokenResponse> {
		try {
	...
	async oAuthLogin(payload: OAuthLoginInput, res: Response): Promise<any> {
		const user = await this.userService.checkIfUserExist(payload.email);
		if (!user) {
